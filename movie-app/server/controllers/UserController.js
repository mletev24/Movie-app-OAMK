...
const postRegistration = async(req,res,next) => {
    try {
        ...
        const userFromDb = await insertUser(req.body.email, req.body.password, createdAt)
        const user = userFromDb.rows[0]
        return res.status(201).json(createUserObject(user.id,user.email, user.createdAt))
    } catch (error) {
        return next(error)
    }
}
const createUserObject = (id, email, createdAt, token=undefined) => {
    return {
        "id": id,
        "email": email,
        "createdAt": createdAt,
